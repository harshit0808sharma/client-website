...
const SalonHeader = () => {
    ...
    const getLinkClasses = (href) => {
        const baseClasses = 'relative px-3 py-2 text-sm font-medium transition-colors duration-300 group';
        const activeClasses = 'text-gray-900';
        const inactiveClasses = 'text-gray-600 hover:text-gray-900';

        return pathname === href ? `${baseClasses} ${activeClasses}` : `${baseClasses} ${inactiveClasses}`;
    };
                    ...
                    <div className="hidden md:flex items-center space-x-8">
                        {navLinks.map((link) => (
                            ...
                            >
                                ...
                                <span className={`absolute bottom-0 left-0 h-0.5 bg-current transition-all duration-300 ease-out group-hover:w-full ${pathname === link.href ? 'w-full' : 'w-0'}`}></span>
                            </Link>
                        ))}
